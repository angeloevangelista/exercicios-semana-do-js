...
const countOfNumbersLessThan501 = crazyNumbers.reduce(
  (accumulator, number) => (number < 501 ? accumulator + 1 : accumulator),
  0
);

console.log(countOfNumbersLessThan501);
...
const tarantinoMoviesReleasedBefore2000 = tarantinoMovies.filter(
  (movie) => movie.release < 2000
);

console.table(tarantinoMoviesReleasedBefore2000);
...
const gamesNamesString = cart.reduce(
  (accumulatedNames, currentGame) =>
    `${accumulatedNames}\n${itemPrefix}${currentGame.name}`,
  ""
);

console.log(gamesNamesString);
